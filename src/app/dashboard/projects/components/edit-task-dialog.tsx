...
export function EditTaskDialog({ open, onOpenChange, projectId, task, teamMemberIds }: EditTaskDialogProps) {
    ...
    const onSubmit = async (values: TaskFormValues) => {
        ...
        try {
            ...
            // If status changed to DONE, add completedAt
            if (values.status === 'DONE' && task.status !== 'DONE') {
                updateData.completedAt = Timestamp.now();
            }
            // If status changed from DONE to something else, remove completedAt
            if (values.status !== 'DONE' && task.status === 'DONE') {
            ...
            toast({
                title: 'Алдаа',
                description: 'Таск шинэчлэхэд алдаа гарлаа.',
                variant: 'destructive',
            });
        } finally {
                                        ...
                                        {teamMembers.length === 0 ? (
                                            ...
                                            teamMembers.filter(e => e.id).map((employee) => (
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <label
                                                        htmlFor={`edit-assignee-${employee.id}`}
